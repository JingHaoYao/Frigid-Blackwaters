...
public class FrogmanOrbCaster : Enemy
{
    ...
    {
        GameObject instant = Instantiate(orbProjectile, pickRandomPositionToSpawnOrb(), Quaternion.Euler(0, 0, Random.Range(0, 360)));
        instant.GetComponent<ProjectileParent>().instantiater = this.gameObject;
    }

    Vector3 pickRandomPositionToSpawnOrb()
    {
        float randomAngle = Random.Range(0, Mathf.PI * 2);
        Vector3 positionToReturn = PlayerProperties.playerShipPosition + new Vector3(Mathf.Cos(randomAngle), Mathf.Sin(randomAngle)) * 2.5f;
        while(Mathf.Abs(positionToReturn.x - mainCamera.transform.position.x) > 8.5f || Mathf.Abs(positionToReturn.y - mainCamera.transform.position.y) > 8.5f)
        {
            randomAngle = Random.Range(0, Mathf.PI * 2);
            positionToReturn = PlayerProperties.playerShipPosition + new Vector3(Mathf.Cos(randomAngle), Mathf.Sin(randomAngle)) * 2.5f;
        }
