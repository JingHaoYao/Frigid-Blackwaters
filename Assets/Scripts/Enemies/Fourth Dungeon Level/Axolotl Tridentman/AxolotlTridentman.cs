...
public class AxolotlTridentman : Enemy
{
    ...
    void Update()
    {
        ...
        if(health < maxHealth)
        {
            ...
            {
                this.heal(1);
                GameObject healParticlesInstant = Instantiate(healParticles, transform.position, Quaternion.identity);
                healParticlesInstant.GetComponent<FollowObject>().objectToFollow = this.gameObject;
                healPeriod = 0;
            }
            else
            {
