...
public class FrogmanSnarer : Enemy
{
    ...
    List<GameObject> snareList = new List<GameObject>();
    ...
    IEnumerator placeSnare()
    {
        ...
        {
            GameObject mineInstant = Instantiate(snare, transform.position, Quaternion.identity);
            mineInstant.GetComponent<ProjectileParent>().instantiater = this.gameObject;
            mineInstant.GetComponent<FrogmanSnare>().snarer = this;
            snareList.Add(mineInstant);
        }
