...
public class BonfireSkeleton : Enemy
{
    ...
    void spawnProjectiles()
    {
        for (int i = 0; i < 4; i++)
        {
            ...
            GameObject projectileInstant = Instantiate(fireBall, transform.position + Vector3.up * 1.25f, Quaternion.Euler(0, 0, angle * Mathf.Rad2Deg));
            projectileInstant.GetComponent<ProjectileParent>().instantiater = this.gameObject;
            projectileInstant.GetComponent<FrogLanternFireProjectile>().angleTravel = i * 90;
        }
    }
    ...
    IEnumerator launchProjectiles()
    {
        ...
        animator.SetTrigger("Burst" + whatView);
        attackAudio.Play();
        auraController.fadeInLights(0.2f);
        if (invisController.isUnderLight == false)
        {
    ...
    Vector3 pickRandPos()
    {
        ...
        {
            randX = transform.position.x + Random.Range(2.0f, 3.0f);
            if (Random.Range(0, 2) == 1)
            {
                randY = transform.position.y + Random.Range(2.0f, 3.0f);
            }
            else
            {
                randY = transform.position.y + Random.Range(-3.0f, -2.0f);
            }
        }
        else
        {
            randX = transform.position.x + Random.Range(-3.0f, -2.0f);
            if (Random.Range(0, 2) == 1)
            {
                randY = transform.position.y + Random.Range(2.0f, 3.0f);
            }
