...
public class FrogmanLanternHolder : Enemy
{
    ...
    {
        GameObject bolaInstant = Instantiate(fireProjectile, transform.position + new Vector3(Mathf.Cos(angle), Mathf.Sin(angle) + 0.7f) * 0.5f, Quaternion.Euler(0, 0, angle * Mathf.Rad2Deg));
        bolaInstant.GetComponent<FrogLanternFireProjectile>().angleTravel = angle * Mathf.Rad2Deg;
    ...
    IEnumerator launchFireProjectile()
    {
        ...
        {
            spawnFireProjectile(throwAngle * Mathf.Deg2Rad);
        }
