...
public class MushroomGoliath : Enemy
{
    ...
    void travelLocation()
    {
        ...
        else
        {
            ...
            {
                StartCoroutine(launchBall(PlayerProperties.playerShipPosition, () => StartCoroutine(launchBall(randomPosition()))));
            }
        }
    }
    ...
    Vector3 randomPosition()
    {
        ...
        while(Physics2D.OverlapCircle(randPos, 0.4f, detectLayerMask))
        {
    ...
    IEnumerator launchBall(Vector3 targetPosition, UnityAction onComplete = null)
    {
        isAttacking = true;
        pickView((360 + Mathf.Atan2(targetPosition.y - transform.position.y, targetPosition.x - transform.position.x) * Mathf.Rad2Deg) % 360);
        transform.localScale = new Vector3(4 * mirror, 4);
    ...
    void spawnBomb(Vector3 target)
    {
        GameObject thornBallInstant = Instantiate(sporeBall, transform.position + new Vector3(0, 1.2f), Quaternion.identity);
        thornBallInstant.GetComponent<MushroomGoliathSporeBall>().targetLocation = target;
