...
public class CrystallinePillarEffectCircle : MonoBehaviour
{
    ...
    IEnumerator spawnCrystalLoop()
    {
        while (true)
        {
            ...
            if (Mathf.Abs(currentScale - previousScale) * 0.7f > 1)
            {
                ...
                for(int i = 0; i < 8; i++)
                {
                    Vector3 position = transform.position + new Vector3(Mathf.Cos((i * 45 + offsetAngle) * Mathf.Deg2Rad), Mathf.Sin((i * 45 + offsetAngle) * Mathf.Deg2Rad)) * (transform.localScale.x * 0.7f);
                    if (checkIfPositionIsValid(position))
                    {
                        ...
                        crystalInstant.GetComponent<ProjectileParent>().instantiater = crystalBoss;
                    }
                }
            }
            yield return null;
