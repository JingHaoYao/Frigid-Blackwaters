...
public class PyrotheumThrower : Enemy
{
    ...
    void spawnFlames()
    {
        for(int i = 0; i < 3; i++)
        {
            GameObject fireInstant = Instantiate(pyrotheumBlast, transform.position + returnFireSpawnPosition(), Quaternion.identity);
            fireInstant.GetComponent<ShortPyrotheumBlast>().Initialize(this.gameObject, angleToShip() - 15 + 15 * i);
        }
    }
    ...
    Vector3 returnFireSpawnPosition()
    {
        if(whatView == 1)
        {
            return new Vector3(mirror * 0.71f, 0.278f);
        }
        else if(whatView == 2)
        {
        ...
        {
            return new Vector3(mirror * 0.909f, 0.292f);
        }
        else
        {
            return new Vector3(0, 1.637f);
        }
    }
