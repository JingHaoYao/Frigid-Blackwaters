...
public class RockBubbler : Enemy
{
    ...
    int whatView = 1;
    int mirror = 1;
    [SerializeField] GameObject homingBubble;
    ...
    IEnumerator launchFireProjectile()
    {
        ...
        {
            GameObject homingBubbleInstant = Instantiate(homingBubble, transform.position + Vector3.up * 0.5f, Quaternion.identity);
            homingBubbleInstant.GetComponent<RockBubblerHomingProjectile>().Initialize(this.gameObject);
        }
