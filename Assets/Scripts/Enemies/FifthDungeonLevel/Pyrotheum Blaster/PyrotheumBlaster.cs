...
public class PyrotheumBlaster : Enemy
{
    ...
    IEnumerator launchFireProjectile()
    {
        ...
        {
            float angle = angleToShip();
            Vector3 center = transform.position + Vector3.up + new Vector3(Mathf.Cos(angle * Mathf.Deg2Rad), Mathf.Sin(angle * Mathf.Deg2Rad)) * 0.5f;
            for(int i = 0; i < 3; i++)
            {
                ...
                projectileInstant.GetComponent<PyrotheumBlasterProjectile>().Initialize(this.gameObject, center, i, angle * Mathf.Deg2Rad);
            }
        }
