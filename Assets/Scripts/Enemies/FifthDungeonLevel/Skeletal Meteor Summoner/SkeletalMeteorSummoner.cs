...
public class SkeletalMeteorSummoner : Enemy
{
    ...
    IEnumerator summonMeteor()
    {
        ...
        if(stopAttacking == false)
        {
            for (int i = 0; i < 3; i++)
            {
                Vector3 spawnLocation = PickProjectileLocation();
                GameObject meteorInstant = Instantiate(meteorProjectile, spawnLocation, Quaternion.identity);
                meteorInstant.GetComponent<SkeletalMeteor>().Initialize(spawnLocation, 4f, this.gameObject);
