...
public class TremorMakerUpgradeTile : MonoBehaviour
{
    public TremorMakerUpgradeTile prevTile;
    public TremorMakerUpgradeTile[] nextTiles;
                ...
                if (prevTile == null)
                {
                    ...
                    if (unlocked == false)
                    {
                        if (PlayerUpgrades.numberSkillPoints >= skillPointsRequirement)
                        {
                            ...
                            upgraded = true;
                            lockedIcon.SetActive(false);
                            PlayerUpgrades.tremorMakerUpgrades.Add(upgradeID);
                        ...
                        if (unlocked == false)
                        {
                            if (PlayerUpgrades.numberSkillPoints >= skillPointsRequirement)
                            {
                                ...
                                if (prevTile.nextTiles.Length > 0)
                                {
                                    ...
                                    {
                                        foreach (TremorMakerUpgradeTile tile in prevTile.nextTiles)
                                        {
                                            if (tile != this)
                                            {
    ...
    bool checkIfUnlocked(TremorMakerUpgradeTile[] tiles)
    {
        foreach (TremorMakerUpgradeTile tile in tiles)
        {
