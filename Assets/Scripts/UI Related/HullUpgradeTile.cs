...
public class HullUpgradeTile : MonoBehaviour {
    public HullUpgradeTile prevTile;
    public HullUpgradeTile[] nextTiles;
                ...
                if (prevTile == null)
                {
                    ...
                    if (unlocked == false)
                    {
                        if (PlayerUpgrades.numberSkillPoints >= skillPointsRequirement)
                        {
                            ...
                            upgraded = true;
                            lockedIcon.SetActive(false);
                            PlayerUpgrades.hullUpgrades.Add(upgradeID);
                        ...
                        if (unlocked == false)
                        {
                            if (PlayerUpgrades.numberSkillPoints >= skillPointsRequirement)
                            {
                                ...
                                if (prevTile.nextTiles.Length > 0)
                                {
                                    ...
                                    {
                                        foreach (HullUpgradeTile tile in prevTile.nextTiles)
                                        {
                                            if (tile != this)
                                            {
    ...
    bool checkIfUnlocked(HullUpgradeTile[] tiles)
    {
        foreach (HullUpgradeTile tile in tiles)
        {
        ...
        if (upgraded == true && operationApplied == false)
        {
            if (nextTiles.Length > 1)
            {
                if (checkIfUnlocked(nextTiles) == false)
                {
                    operationApplied = true;
                    upgraded = false;
                    PlayerUpgrades.hullUpgrades.Remove(upgradeID);
                    imageIcon.color = new Color(1, 1, 1, 0.63f);
