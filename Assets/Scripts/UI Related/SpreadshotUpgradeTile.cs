...
public class SpreadshotUpgradeTile : MonoBehaviour
{
    public SpreadshotUpgradeTile prevTile;
    public SpreadshotUpgradeTile[] nextTiles;
                ...
                if (prevTile == null)
                {
                    ...
                    if (unlocked == false)
                    {
                        if (PlayerUpgrades.numberSkillPoints >= skillPointsRequirement)
                        {
                            ...
                            upgraded = true;
                            lockedIcon.SetActive(false);
                            PlayerUpgrades.spreadshotUpgrades.Add(upgradeID);
                        ...
                        if (unlocked == false)
                        {
                            if (PlayerUpgrades.numberSkillPoints >= skillPointsRequirement)
                            {
                                ...
                                if (prevTile.nextTiles.Length > 0)
                                {
                                    ...
                                    {
                                        foreach (SpreadshotUpgradeTile tile in prevTile.nextTiles)
                                        {
                                            if (tile != this)
                                            {
    ...
    bool checkIfUnlocked(SpreadshotUpgradeTile[] tiles)
    {
        foreach (SpreadshotUpgradeTile tile in tiles)
        {
        ...
        if (upgraded == true && operationApplied == false)
        {
            if (nextTiles.Length > 1)
            {
                if (checkIfUnlocked(nextTiles) == false)
                {
                    operationApplied = true;
                    upgraded = false;
                    PlayerUpgrades.spreadshotUpgrades.Remove(upgradeID);
                    imageIcon.color = new Color(1, 1, 1, 0.63f);
