...
public class GlaiveLauncherUpgradeTile : MonoBehaviour
{
    public GlaiveLauncherUpgradeTile prevTile;
    public GlaiveLauncherUpgradeTile[] nextTiles;
                ...
                if (prevTile == null)
                {
                    ...
                    if (unlocked == false)
                    {
                        if (PlayerUpgrades.numberSkillPoints >= skillPointsRequirement)
                        {
                            ...
                            upgraded = true;
                            lockedIcon.SetActive(false);
                            PlayerUpgrades.glaiveLauncherUpgrades.Add(upgradeID);
                        ...
                        if (unlocked == false)
                        {
                            if (PlayerUpgrades.numberSkillPoints >= skillPointsRequirement)
                            {
                                ...
                                if (prevTile.nextTiles.Length > 0)
                                {
                                    ...
                                    {
                                        foreach (GlaiveLauncherUpgradeTile tile in prevTile.nextTiles)
                                        {
                                            if (tile != this)
                                            {
    ...
    bool checkIfUnlocked(GlaiveLauncherUpgradeTile[] tiles)
    {
        foreach (GlaiveLauncherUpgradeTile tile in tiles)
        {
