...
public class FireworkUpgradeTile : MonoBehaviour {
    public FireworkUpgradeTile prevTile;
    public FireworkUpgradeTile[] nextTiles;
                ...
                if (prevTile == null)
                {
                    ...
                    if (unlocked == false)
                    {
                        if (PlayerUpgrades.numberSkillPoints >= skillPointsRequirement)
                        {
                            ...
                            upgraded = true;
                            lockedIcon.SetActive(false);
                            PlayerUpgrades.fireworkUpgrades.Add(upgradeID);
                        ...
                        if (unlocked == false)
                        {
                            if (PlayerUpgrades.numberSkillPoints >= skillPointsRequirement)
                            {
                                ...
                                if (prevTile.nextTiles.Length > 0)
                                {
                                    ...
                                    {
                                        foreach (FireworkUpgradeTile tile in prevTile.nextTiles)
                                        {
                                            if (tile != this)
                                            {
    ...
    bool checkIfUnlocked(FireworkUpgradeTile[] tiles)
    {
        foreach (FireworkUpgradeTile tile in tiles)
        {
        ...
        if (upgraded == true && operationApplied == false)
        {
            if (nextTiles.Length > 1)
            {
                if (checkIfUnlocked(nextTiles) == false)
                {
                    operationApplied = true;
                    upgraded = false;
                    PlayerUpgrades.fireworkUpgrades.Remove(upgradeID);
                    imageIcon.color = new Color(1, 1, 1, 0.63f);
