...
public class PodFlyersUpgradeTile : MonoBehaviour
{
    public PodFlyersUpgradeTile prevTile;
    public PodFlyersUpgradeTile[] nextTiles;
                ...
                if (prevTile == null)
                {
                    ...
                    if (unlocked == false)
                    {
                        if (PlayerUpgrades.numberSkillPoints >= skillPointsRequirement)
                        {
                            ...
                            upgraded = true;
                            lockedIcon.SetActive(false);
                            PlayerUpgrades.podFlyersUpgrades.Add(upgradeID);
                        ...
                        if (unlocked == false)
                        {
                            if (PlayerUpgrades.numberSkillPoints >= skillPointsRequirement)
                            {
                                ...
                                if (prevTile.nextTiles.Length > 0)
                                {
                                    ...
                                    {
                                        foreach (PodFlyersUpgradeTile tile in prevTile.nextTiles)
                                        {
                                            if (tile != this)
                                            {
    ...
    bool checkIfUnlocked(PodFlyersUpgradeTile[] tiles)
    {
        foreach (PodFlyersUpgradeTile tile in tiles)
        {
