...
public class CannonUpgradeTile : MonoBehaviour {
    public CannonUpgradeTile prevTile;
    public CannonUpgradeTile[] nextTiles;
                ...
                if (prevTile == null)
                {
                    ...
                    if (unlocked == false)
                    {
                        if (PlayerUpgrades.numberSkillPoints >= skillPointsRequirement)
                        {
                            ...
                            upgraded = true;
                            lockedIcon.SetActive(false);
                            PlayerUpgrades.cannonUpgrades.Add(upgradeID);
                        ...
                        if (unlocked == false)
                        {
                            if (PlayerUpgrades.numberSkillPoints >= skillPointsRequirement)
                            {
                                ...
                                if (prevTile.nextTiles.Length > 0)
                                {
                                    ...
                                    {
                                        foreach (CannonUpgradeTile tile in prevTile.nextTiles)
                                        {
                                            if (tile != this)
                                            {
    ...
    bool checkIfUnlocked(CannonUpgradeTile[] tiles)
    {
        foreach (CannonUpgradeTile tile in tiles)
        {
        ...
        if (upgraded == true && operationApplied == false)
        {
            if (nextTiles.Length > 1)
            {
                if (checkIfUnlocked(nextTiles) == false)
                {
                    operationApplied = true;
                    upgraded = false;
                    PlayerUpgrades.cannonUpgrades.Remove(upgradeID);
                    PlayerUpgrades.numberSkillPoints += skillPointsRequirement;
                    imageIcon.color = new Color(1, 1, 1, 0.63f);
